...
def generate_pages_recursive(dir_path_content, template_path, dest_dir_path):
    ...
    for file in content.iterdir():
        if file.is_file() and file.name.endswith('.md'):
            if not generate_page((content / file.name).resolve(), template.resolve(), (destination / file.name.replace('.md', '.html')).resolve()):
                raise Exception(f'Error: generate_page failed for ${file.resolve()}')
        elif file.is_dir():
            if not generate_pages_recursive((content / file.name).resolve(), template.resolve(), (destination / file.name).resolve()):
